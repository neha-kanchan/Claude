...
export function buildRouter() {
  ...
  r.get('/bootstrap', auth, w(async (req, res) => {
    ...
    for (const name of Object.keys(COLLECTIONS)) {
      ...
      out[name] = await listAll(req.env, name);
    }
  ...
  r.post('/:collection', auth, w(async (req, res) => {
    ...
    await audit(req.env, req.user, created ? 'CREATE' : 'UPDATE', COLLECTION_PAGE[col] || col, rec.id, 'Via REST API');
  ...
  r.put('/:collection/:id', auth, w(async (req, res) => {
    ...
    await audit(req.env, req.user, 'UPDATE', COLLECTION_PAGE[col] || col, req.params.id, 'Via REST API');
  ...
  r.delete('/:collection/:id', auth, w(async (req, res) => {
    ...
    await audit(req.env, req.user, 'DELETE', COLLECTION_PAGE[col] || col, req.params.id, 'Via REST API');
    res.status(204).end();
  }));
